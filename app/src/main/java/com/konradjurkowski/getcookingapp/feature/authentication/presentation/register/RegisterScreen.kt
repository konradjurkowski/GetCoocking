...
fun RegisterScreen(
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            item {
                Text(
                    stringResource(id = R.string.registration_header),
                ...
                AnimatedVisibility(
                    visible = !state.isPasswordValid || !state.isPasswordsTheSame
                ) {
                    Text(
                        text = stringResource(id =
                        if (!state.isPasswordValid) R.string.password_not_correct
                        else R.string.password_are_not_the_same
                        ),
                        style = MaterialTheme.typography.h6,
                        color = MaterialTheme.colors.error
                    )
                ...
                Spacer(modifier = Modifier.height(screenHeight * 0.03f))
                Text(
                    text = stringResource(id = R.string.re_password_label),
                    style = MaterialTheme.typography.h6
                )
                InputTextField(
                    modifier = Modifier.fillMaxWidth(),
                    ...
                    onTextChange = {
                        viewModel.onRepeatedPasswordChange(it)
                    },
                    placeholder = stringResource(id = R.string.re_password_hint),
                    keyboardType = KeyboardType.Password,
                    visualTransformation = PasswordVisualTransformation()
                )
                AnimatedVisibility(
